...
def run(args):
    ...
    item_commonality = torch.nn.Embedding(num_embeddings=server.args['num_items'], embedding_dim=server.args['item_hidden_dim'])
    ...
    for communication_round in range(1, args['num_rounds']+1):
        ...
        server.train_on_round(item_commonality, participants)
        ...
        sns.heatmap(torch.abs(updated_params['item_commonality.weight'][:, :]))

        average_figure = plt.figure(figsize=(10, 10))
        sns.heatmap(torch.abs(avg_item_personal['item_personality.weight'][:, :]))

        first_client_personal_figure = plt.figure(figsize=(10, 10))
        sns.heatmap(torch.abs(server.user_context[0]['state_dict']['item_personality.weight'][:, :]))

        first_client_common_figure = plt.figure(figsize=(10, 10))
        sns.heatmap(torch.abs(server.user_context[0]['state_dict']['item_commonality.weight'][:, :]))
