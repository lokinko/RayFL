...
def run(args):
    ...
    for communication_round in range(1, args['num_rounds']+1):
        ...
        global_params = server.aggregate(participants, ['item_embedding.weight'])

        # update user context
        for _, user_context in server.user_context.items():
            user_context['state_dict']['item_embedding.weight'] = copy.deepcopy(
                                global_params['item_embedding.weight'])

        # update global model
        server_params = copy.deepcopy(server.global_model.state_dict())
        server_params['item_embedding.weight'] = global_params['item_embedding.weight'].data
        ...
        sns.heatmap(torch.abs(global_params['item_embedding.weight'][:, :]))
        ...
        sns.heatmap(torch.abs(server.user_context[client_id]['state_dict']['item_embedding.weight'][:, :]))
